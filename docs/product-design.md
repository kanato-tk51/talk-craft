...
9. 2026-07-26時点の`npm audit`は、本番依存にOpenNext/Next.js経由のhigh 8件、開発依存を含めてmoderate 4件を追加で報告する。提案される`--force`修正は主要パッケージを非互換な旧版へ戻すため適用せず、開発サーバーを外部公開しないことと、OpenNext/Next.js/Drizzleの修正版追跡で対応する。
...
| DB | Cloudflare D1（SQLite） | Workersからbindingで直接利用でき、個人利用では無料枠内に収まりやすく、本番接続情報をローカルへ保持せずに運用できる。 |
...
| 認証 | Cloudflare Access + Access JWT再検証 | 本人のメール1件だけをEdgeで許可し、アプリ側でもissuer、audience、メールを検証する。 |
| テスト | Vitest、Testing Library、Playwright | ドメイン単体、コンポーネント、主要ユーザーフローを層別に検証する。 |
| ローカル | Node.js 22、npm、Wrangler local D1 | `.wrangler/state`のローカルSQLiteで、本番データや接続情報なしに開発できる。 |
| CI | GitHub Actions | `lint`、`typecheck`、`test`、`build`、migration検査をPR単位で行う。 |

## 4. システム構成

初期は1つのNext.jsアプリと1つのCloudflare D1からなるモジュラーモノリスとする。
...
すべてのIDはアプリで生成するUUID文字列、日時はUnix epochミリ秒のSQLite `integer`、自由文はUTF-8の`text`とする。構造化配列・オブジェクトはJSON modeの`text`に保存する。列挙値はアプリだけでなくDBのcheckでも制約する。更新競合に備え、編集対象には`version`を持たせる。
...
| `expressions` | ユーザーの再利用可能な表現ライブラリ。配列項目はJSON文字列として構造化する。 |
...
9. commit: user確認、version一致、所有者確認後、D1のatomic batchでdomain tableへ反映。
...
公開MVPはCloudflare AccessのOne-time PINで本人メール1件だけを許可する。アプリは`Cf-Access-Jwt-Assertion`の署名、issuer、audience、メール完全一致を再検証し、検証済みメールからD1上のユーザーを初回アクセス時に自動作成する。

認可はAccessだけに依存しない。各Server Action、Route Handler、application serviceでactorを解決し、repositoryに`actorUserId`を必須引数として渡す。他人のUUIDを指定されても`WHERE id = ? AND user_id = ?`で取得し404を返す。UUIDは認可境界ではない。

ローカル開発では`DEV_USER_ID`のユーザーをローカルD1へ自動作成するが、production runtimeでは固定ユーザーfallbackを許可しない。
...
- D1はWorker bindingで接続し、接続URLやDBパスワードを持たない。将来のprovider tokenはsecret managerへ置き、token列はapplication-level encryptionする。
...
3. Authentication: Cloudflare Access JWT検証、所有者スコープ、初回ユーザー自動登録。
4. Practice tracking: status transition、開始/終了、外部URL、使用表現check。
5. Import staging: paste/file、raw保存、size/JSON/schema検証、preview。
6. Import commit/review: D1 atomic batch反映、復習表示、provenance、ユーザー編集。
...
- F-02 Cloudflare D1 binding、Drizzle SQLite client/schema/migration。
...
- A-01 Cloudflare Access JWT検証と本番環境変数validation。
- A-02 初回ユーザー自動登録とserver-side actor解決。
...
- I-06 D1 atomic batch commitとreplace policy。
...
- Repository integration: ローカルD1でcascade、unique、batch rollback、user isolation、再import置換。
...
前提はNode.js 22以上とnpm 11です。Dockerは不要です。
...
ローカルDBは`.wrangler/state`に保存され、開発ユーザーは初回アクセス時に自動作成される。migrationは`npm run db:generate`で生成し、SQLをreviewしてからcommitする。適用はローカルが`npm run db:migrate`、本番が`npm run db:migrate:remote`であり、本番で`db:push`は使わない。

## 22. YouTube字幕教材（2026-07-15追加）

### 利用フロー

1. ユーザーはYouTubeの動画説明欄から「文字起こしを表示」を開き、章見出し・タイムスタンプを含む英語字幕をコピーする。
2. `/youtube/new` でYouTube URL、任意の動画情報、コピーした英語字幕を受け取る。動画IDは許可したYouTube URL形式からのみ抽出し、字幕内のタイムスタンプと英文を解析する。字幕キューは翻訳依頼用の番号付きソース単位へまとめるが、この番号を画面の段落境界には使用しない。
3. 字幕全文を含む翻訳プロンプトをTalk Craftが生成する。ユーザーはプロンプトをコピーしてブラウザ版ChatGPTへ貼り付け、JSON形式の回答をTalk Craftへ戻す。
4. Talk Craftは回答JSONから前後の説明やコードフェンスを機械的に除去し、要約、翻訳段落、重要表現を検証する。段落の欠落・重複・順序、字幕ID、原文一致、時刻、表現数、注釈対象の一致を確認し、不正な回答は保存しない。
5. 詳細画面ではChatGPTが構成した段落単位で英語原文と日本語訳を同じ行に表示する。重要表現は通常文と同じ折り返し規則を持つ赤色・下線付きのインライン注釈とし、クリック時に右側のコメントパネルで意味・ニュアンス・例文を参照できるようにする。狭い画面では同じ情報を画面下部のパネルに表示する。
6. ユーザーは英語原文内をドラッグ選択し、任意の文字列を重要表現として追加できる。ChatGPT由来とユーザー追加のどちらの表現も解説パネルから削除できる。追加・削除時は原文との一致、重複、所有者、更新versionを検証し、ChatGPTの回答を登録し直してもユーザー追加表現を保持する。
7. 詳細画面では `youtube-nocookie.com` の埋め込みプレーヤーを表示し、ページを離れずに動画を再生できるようにする。投稿者が埋め込みを禁止した動画にはYouTube本体へのリンクを代替導線として残す。
8. 所有するYouTube教材は確認ダイアログを経て削除できる。削除対象は `user_id` でスコープし、教材行に含まれる字幕・翻訳・重要表現・AI原文を一括削除した後、教材一覧へ戻す。

### 保存モデル

`youtube_materials` はユーザーとYouTube動画IDの組をuniqueとし、動画メタデータ、字幕原文、番号付きソースブロック、翻訳プロンプトとversion、日本語要約、ChatGPTが返した英語段落と日本語訳、AI／ユーザー由来の重要表現、監査用の回答原文を保持する。更新競合用の `version` と翻訳登録完了を示す `translated_at` を持ち、ユーザー削除時はcascadeする。

### 外部仕様と制約

YouTube Data APIの `captions.download` は動画を編集できるユーザーの認可を必要とするため、任意の公開動画を対象にする本機能では利用しない。サーバーからYouTubeの公開ページや非公式字幕インターフェースへの取得リクエストも行わない。ユーザーがコピーした字幕だけを入力とし、タイムスタンプの欠落・逆転、空字幕、文字数超過を検証してから保存する。動画URLは動画IDの抽出と再生リンクの生成にだけ利用し、入力されたURLへサーバーからアクセスしない。

字幕本文とChatGPTの回答はアプリケーションログへ書き込まない。

## 実装開始前の明示事項

### 採用する技術と採用理由

Next.js/TypeScriptのモジュラーモノリス、Cloudflare D1、Drizzle、Zodを採用する。1言語・1repository・1deployableで小人数の変更速度を保ちつつ、DB制約、application service、adapter portで将来の連携方式を分離できるためである。

### 採用しなかった主な選択肢

- マイクロサービス: 現時点の負荷・チーム規模ではdeploy、監視、整合性管理のコストが先行する。
- Firebase/Firestore中心: session-reviewの関連データ、整合性制約、exportにはSQLite互換のD1が自然。
- Prisma 7: 良い選択肢だが、現行ローカルNode 20.18が要求する20.19を満たさない。無理なruntime更新を初手の前提にしない。
- Better Auth / Auth.js: 本人1名だけが利用する現在の要件では、Cloudflare Accessとアプリ内JWT再検証で認証境界を一元化できるため導入しない。
...
2. `wrangler.jsonc`, `.dev.vars.example`: 再現可能なローカルD1環境。
